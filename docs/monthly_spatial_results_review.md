...
The local reviewer validates the original portable spatial archive and optionally adds the successful Listeria numerical recovery and the separately verified Shigella optimizer restart. It does not overwrite original results or mark an incomplete comparison complete.

```bash
python3 scripts/review_monthly_spatial_results.py /path/to/broader_combinations_RUN.tar.gz --recovery /path/to/monthly_spatial_recovery_RUN.tar.gz --restart /path/to/shigella_numerical_restart_RUN.tar.gz --output output/spatial_review_RUN_LOCAL
```
...
The separate restart must link to both the original experiment and its preceding recovery. Completion fills a previously failed cell; it does not select a model. If both the recreated initial fit and its optimizer restart pass, that establishes successful execution on this attempt, not evidence that the restart caused the improvement. Preserve the earlier failure in the numerical history.
...
Local checks include synthetic integrity tests and a complete review of the returned original, recovery and restart archives. Private findings and plots remain under output/ and are excluded from publication.
