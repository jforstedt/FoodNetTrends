...
The first [real-data monthly comparison](monthly_comparison.md) now adopts continuity explicitly as an exploratory assumption, with a hashed site-month ledger and `coverage_certified=false`. This permits that conditional comparison without claiming the missing operational evidence has been obtained. Source inventories remain unverified, and stronger acceptance claims still require coverage and statistical review.
