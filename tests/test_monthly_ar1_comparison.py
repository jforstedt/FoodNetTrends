...
import launch_monthly_ar1_comparison as m
...
class SensitivityTests(unittest.TestCase):
    ...
    def test_six_new_fits_and_reused_reference(self):
        ...
        self.assertEqual(p['temporal_model'],'ar1');self.assertEqual(p['marginal_sd_upper'],1)
        self.assertTrue(all(t['seasonal'] for t in p['tasks']));self.assertEqual(len({t['seed'] for t in p['tasks']}),6)
        for t in p['tasks']:
            self.assertIn('run_monthly_ar1_comparison.R',t['command'][10]);self.assertIn(m.BASELINE,t['baseline'])
    ...
    def test_python36(self):ast.parse((ROOT/'scripts/launch_monthly_ar1_comparison.py').read_text(),feature_version=(3,6))
