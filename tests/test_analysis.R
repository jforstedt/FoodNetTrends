...
co_cases <- data.frame(year=2001:2025,state='CO',county='ADAMS',siteid='CO',pathogen='SALMONELLA')
r <- prepare_analysis_inputs(co_cases,co_history,co_history,'SALMONELLA')
...
stopifnot(inherits(try(prepare_analysis_inputs(co_cases,bad_history,bad_history,'SALMONELLA'),silent=TRUE),'try-error'))
...
stopifnot(inherits(try(prepare_analysis_inputs(bad_cases,co_history,co_history,'SALMONELLA'),silent=TRUE),'try-error'))
