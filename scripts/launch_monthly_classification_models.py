...
import argparse,csv,json,math,re,shlex,shutil,subprocess,sys,tarfile,time
...
def progress(message,started=None):
 elapsed='' if started is None else ' ({:.1f}s)'.format(time.monotonic()-started)
 print(datetime.now().strftime('[%Y-%m-%d %H:%M:%S] ')+message+elapsed,flush=True)

def verify_source_metadata(origin,previous,digest):
 if sha(origin/'plan.json')!=digest or previous.get('version')!='classification_monthly_preparation_v1' or previous.get('verified') is not True or previous.get('models_fitted') is not False or previous.get('incidence_adjustment') is not False:raise ValueError('Invalid source preparation protocol')
 if [t['id'] for t in previous.get('tasks',[])]!=list(prep.PATHOGENS):raise ValueError('Invalid source preparation matrix')
 if not isinstance(previous.get('inputs'),dict) or not previous['inputs']:raise ValueError('Missing historical source provenance')
 # Recheck the frozen preparation code/reports, not archived raw inputs that this fit does not consume.
 bound={}
 for absolute,h in previous['inputs'].items():
  if absolute.startswith(str(origin)+'/'):
   p=inside(origin,absolute[len(str(origin))+1:])
   if sha(p)!=h:raise ValueError('Changed source preparation snapshot')
   bound[str(p)]=h
 return bound

def prepare(root,dest,verified=True):
 root=Path(root).resolve();dest=Path(dest).resolve();origin=root/'output'/SOURCE/'classification';inputs={};source_tasks={};started=time.monotonic()
 progress('Preparing monthly classification experiment')
 if dest.exists():raise FileExistsError(str(dest))
 if verified:
  progress('Checking source preparation identity and historical lineage')
  previous=json.loads((origin/'plan.json').read_text());digest=sha(origin/'plan.json');inputs.update(verify_source_metadata(origin,previous,digest))
  ...
  for t in previous['tasks']:
   stage=time.monotonic();progress('Checking '+t['id']+' derived panels, annual support and output hashes')
   ...
   support=str(Path(t['annual_support']));expected_support=previous['inputs'].get(support)
   if not expected_support or sha(support)!=expected_support:raise ValueError('Annual support differs from source preparation')
   prep.validate(work,t);bound={str(work/'task_status.json'):sha(work/'task_status.json'),support:expected_support}
   ...
   source_tasks[t['id']]=bound;progress('Verified '+t['id'],stage)
 stage=time.monotonic();progress('Checking frozen prior calibration and model source hashes')
 prior_inputs=verify_prior(root) if verified else {}
 progress('Prior calibration checks complete',stage);progress('Copying model, graph and prior snapshots')
 ...
 if verified:
  stage=time.monotonic();progress('Hashing runtime container '+str(container));inputs[str(container)]=sha(container);progress('Runtime container verified',stage)
 tasks=matrix()
 for t in tasks:
 ...
 if verified:
  plan['source_provenance']=previous['inputs'];plan['source_provenance_status']='HISTORICAL_RAW_INPUT_HASHES_RETAINED_NOT_REHASHED';plan['derived_inputs_reverified']=True
...
def launch(root,dest,pin=None,pin_digest=None):
 if pin:verify_submission_pin(Path(pin),pin_digest,root,dest)
 prepare(root,dest)
 if pin:verify_submission_pin(Path(pin),pin_digest,root,dest)
 progress('Submitting 144 model tasks')
 digest=sha(dest/'plan.json');base=['qsub','-terse','-V','-cwd','-S','/bin/bash','-j','y'];submission={}
...
def verify_submission_pin(folder,digest,root,dest=None):
 manifest=folder/'submission_pin.json'
 if not digest or sha(manifest)!=digest:raise ValueError('Preparation code pin changed')
 pin=json.loads(manifest.read_text())
 if dest is not None and pin.get('destination')!=str(Path(dest).resolve()):raise ValueError('Wrong preparation destination')
 if pin.get('root')!=str(Path(root).resolve()) or not pin.get('files'):raise ValueError('Wrong preparation code root')
 for name,h in pin['files'].items():
  if sha(inside(folder,name))!=h or sha(inside(Path(root),name))!=h:raise ValueError('Preparation code changed since submission: '+name)
 return pin
...
def submit_preparation(root,dest):
 ...
 paths += [root/'analysis_configs/county_pilot'/n for n in ('counties.csv','edges.csv','provenance.json')]
 paths += sorted((root/'analysis_configs/monthly_classification_priors').glob('*'))
 if not (root/'analysis_configs/monthly_classification_priors/manifest.json').is_file():raise ValueError('Missing frozen prior manifest')
 for source in paths:
  if not source.is_file() or source.is_symlink():raise ValueError('Missing or linked preparation source')
  relative=str(source.relative_to(root));target=folder/relative;target.parent.mkdir(parents=True,exist_ok=True);shutil.copyfile(str(source),str(target));files[relative]=sha(target)
 write(folder/'submission_pin.json',dict(root=str(root),destination=str(dest),files=files));digest=sha(folder/'submission_pin.json');q=shlex.quote
 script=folder/'prepare.sh';script.write_text('#!/bin/bash\nset -euo pipefail\nexec python3 '+q(str(folder/'scripts/launch_monthly_classification_models.py'))+' --launch '+q(str(dest))+' --root '+q(str(root))+' --preparation-pin '+q(str(folder))+' --pin-digest '+q(digest)+'\n')
 logfile=Path(str(dest)+'_preparation.log')
 job=subprocess.check_output(['qsub','-terse','-V','-cwd','-S','/bin/bash','-j','y','-N','foodnet_class_prepare','-pe','smp','1','-l','h_rt=04:00:00,h_rss=8192M,mem_free=8192M,h_vmem=16G','-o',str(logfile),str(script)],universal_newlines=True).strip()
 write(folder/'submission.json',dict(preparation=job,destination=str(dest),log=str(logfile),pin_sha256=digest))
 if not re.match(r'^(\d+)(?:[.\s]|$)',job):raise ValueError('Unknown qsub response; inspect queue before resubmitting')
...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('--prepare-only',action='store_true');p.add_argument('--worker');p.add_argument('--collect');p.add_argument('--task');p.add_argument('--digest');p.add_argument('--launch');p.add_argument('--root');p.add_argument('--preparation-pin');p.add_argument('--pin-digest');a=p.parse_args()
 ...
 if a.launch:
  if not a.root or not a.preparation_pin or not a.pin_digest:p.error('Missing scheduled preparation identity')
  launch(Path(a.root),Path(a.launch),a.preparation_pin,a.pin_digest);return 0
 ...
 submit_preparation(root,dest);return 0
