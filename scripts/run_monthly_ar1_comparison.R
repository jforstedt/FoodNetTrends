...
run_ar1_comparison <- function(candidate,audit,cutoff,out,seed,draws=2000L) {
 ...
 warnings<-character()
 fit<-withCallingHandlers(fit_monthly_model(d,cutoff*12+11L,TRUE,threads=4L,coverage='EXPLORATORY_ASSUMED_CONTINUOUS',rate_center=.0002,temporal_model='ar1'),warning=function(w){warnings<<-c(warnings,conditionMessage(w))})
 ...
 audit_monthly_saved(fitpath,predpath,cutoff,TRUE,out,seed,draws=draws,expected_temporal='ar1')
 ...
 write.csv(data.frame(new_fit=TRUE,temporal_model='ar1',marginal_sd_upper=1,rho_internal_mean=log(19),rho_internal_sd=1.5,seasonal=TRUE,rate_center=.0002,coverage_certified=FALSE),file.path(out,'sensitivity_settings.csv'),row.names=FALSE)
}
if(sys.nframe()==0L){a<-commandArgs(TRUE);if(length(a)!=5L)stop('Usage: CANDIDATE AUDIT CUTOFF OUT SEED');run_ar1_comparison(a[1],a[2],as.integer(a[3]),a[4],as.integer(a[5]))}
