...
raw_review <- function(rawpath,cleanpath,reportpath,audit,out,pathogen="SALMONELLA") {
  ...
  tryCatch({
    ...
    if(pathogen=='LISTERIA' && !'cste'%in%names(raw))stop('Missing raw CSTE field')
    raw<-raw[unique(c(required,intersect('cste',names(raw))))]
    ...
    aliases<-unique(as.character(mapping$original[norm(mapping$standardized)==pathogen]))
    if(!length(aliases))stop('No recorded mappings for ',pathogen)
    ...
    if(pathogen=='LISTERIA')raw$removal_hypothesis[norm(raw$cste)!='YES']<-'Listeria_CSTE_not_YES'
    ...
    writeLines(c(if(all(cmp$difference==0))'RAW_CLEAN_DEFAULT_HYPOTHESIS_MATCH' else 'RAW_CLEAN_REVIEW_REQUIRED',
      ...
      paste('Scope uses',pathogen,'aliases from the recorded preprocessing mapping, not new fuzzy matching.'),
