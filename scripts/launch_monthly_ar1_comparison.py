#!/usr/bin/env python3
"""Six stationary AR1 temporal fits; reuse six saved seasonal references."""
import argparse
...
FILES=('launch_monthly_ar1_comparison.py','launch_monthly_comparison.py','launch_saved_monthly_diagnostics.py','run_monthly_ar1_comparison.R','run_monthly_comparison.R','monthly_seasonal_model.R','county_forecast_model.R','fit_county_pilot.R','county_forecast_protocol.py','audit_saved_monthly.R','audit_saved_forecast_sampling.R')
...
def prepare(root,dest,verified=True):
    ...
    protocol=dest/'monthly_ar1_comparison.md';shutil.copyfile(str(root/'docs/monthly_ar1_comparison.md'),str(protocol));inputs[str(protocol)]=sha(protocol)
    tasks=[]
    for pathogen in ('SALMONELLA','CAMPYLOBACTER'):
        for cutoff in (2011,2013,2016):
            ...
            seed=60000000+len(tasks)*1000000
            cmd=['singularity','exec','--cleanenv','--env','OPENBLAS_NUM_THREADS=1,OMP_NUM_THREADS=1,MKL_NUM_THREADS=1','--bind','/scicomp',str(container),'Rscript','--vanilla',str(dest/'scripts/run_monthly_ar1_comparison.R'),sources[pathogen]['candidate'],sources[pathogen]['audit'],str(cutoff),str(dest/name/'result'),str(seed)]
    ...
    plan=dict(version='monthly_ar1_comparison_v1',verified=verified,inputs=inputs,tasks=tasks,new_fits=6,reference_fits_reused=6,temporal_model='ar1',marginal_sd_upper=1,rho_internal_mean=2.9444389791664403,rho_internal_sd=1.5,pc_tail=.01,draws=8000,coverage_certified=False,decision='Exploratory sensitivity; no automatic promotion')
    (dest/'plan.json').write_text(json.dumps(plan,indent=2)+'\n');h=sha(dest/'plan.json');q=shlex.quote;base='python3 '+q(str(dest/'scripts/launch_monthly_ar1_comparison.py'))
...
def verify(dest,plan,digest,task=None):
    if sha(dest/'plan.json')!=digest or not plan.get('verified') or plan.get('version')!='monthly_ar1_comparison_v1' or plan.get('temporal_model')!='ar1' or plan.get('marginal_sd_upper')!=1 or plan.get('rho_internal_sd')!=1.5 or plan.get('rho_internal_mean')!=2.9444389791664403:raise ValueError('Unverified or altered sensitivity plan')
...
def validate(work,task):
    ...
    if len(r)!=1 or r[0]['new_fit']!='TRUE' or r[0]['temporal_model']!='ar1' or float(r[0]['marginal_sd_upper'])!=1 or abs(float(r[0]['rho_internal_mean'])-2.9444389791664403)>1e-10 or float(r[0]['rho_internal_sd'])!=1.5 or r[0]['seasonal']!='TRUE':raise ValueError('Wrong sensitivity setting')
...
def collect(dest,digest):
    ...
    for t in plan['tasks']:
        ...
        try:
            ...
            for name,target in (('stream_scores.csv',scores),('aggregate_tails.csv',tails)):
                for variant,folder in (('ar1',work/'result'),('reference',Path(t['baseline']))):
    ...
    for (task,state,year,stream,variant),r in idx.items():
        ...
        if variant!='ar1' or not b:continue
        paired.append(dict(task=task,pathogen=r['pathogen'],cutoff=r['cutoff'],state=state,year=year,stream=stream,log_score_ar1_minus_reference=float(r['mean_log_score'])-float(b['mean_log_score'])))
    ...
    for (p,c,y,s),rs in groups.items():
        if len(rs)==10:equal.append(dict(pathogen=p,cutoff=c,year=y,stream=s,sites=10,log_score_ar1_minus_reference=sum(r['log_score_ar1_minus_reference'] for r in rs)/10))
    ...
    files=[p for p in dest.rglob('*') if p.is_file() and not p.is_symlink() and p.suffix.lower() in ('.json','.csv','.r','.py','.txt','.sh','.log','.md') and '_INTERNAL' not in p.name and p.name!='report_sha256.json']
...
def main():
    ...
    root=Path(__file__).resolve().parents[1];dest=root/'output'/('monthly_ar1_comparison_'+datetime.now().strftime('%Y%m%d_%H%M%S_%f'))
    ...
    job=subprocess.check_output(base+['-N','foodnet_monthly_ar1','-t','1-6','-pe','smp','4','-l','h_rt=48:00:00,h_rss=53248M,mem_free=53248M,h_vmem=68G','-o',str(dest),str(dest/'run.sh')],universal_newlines=True).strip()
